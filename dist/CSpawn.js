...
Spawn.prototype.spawn = function (body, bodyParts) {
    ...
    if (_.isString(result)) {
        this.room.log('Spawning: ' + result + " with Body: " + bodyParts + " / new sum: " + (this.room.creeps.length + 1));
        if (body == 'BODY_RANGER') Memory.creeps[result].role = 'guard';
        Memory.creeps[result].body = body;
    } else {
        if (result != ERR_BUSY) this.room.log('Spawn error: ' + result + ' while try to spawn ' + bodyParts);
    }
    return result;
};
...
Spawn.prototype.spawnHarvester = function () {
    ...
    if (this.room.hasEnergyCapacity(800)) {
        ...
        if (this.room.creepsDefault.length < 1) this.spawnDefault();
        return;
    }
...
Spawn.prototype.spawnUpgrader = function () {
    ...
    if (this.room.hasEnergyCapacity(2350) && this.room.getCreepsUpgraderCnt() > 2) {
        bodyParts = [WORK, WORK, WORK, WORK, WORK, //500
...
Spawn.prototype.spawnCarrier = function () {
    ...
    if (this.room.extensions.length >= 10) bodyParts = [CARRY, MOVE, CARRY, MOVE, CARRY, MOVE, //300
    CARRY, MOVE, CARRY, MOVE, CARRY, MOVE, //300
    CARRY, MOVE, CARRY, MOVE //200
    ];else if (this.room.extensions.length >= 5) bodyParts = [CARRY, MOVE, CARRY, MOVE, CARRY, MOVE, //300
        CARRY, MOVE, CARRY, MOVE //200
        ];else bodyParts = [CARRY, MOVE, CARRY, MOVE, CARRY, MOVE]; //300
...
Spawn.prototype.spawnRanger = function () {
    ...
    if (false && this.room.extensions.length >= 40) {
        // max: 2300
        bodyParts = [TOUGH, MOVE, TOUGH, MOVE, TOUGH, MOVE, TOUGH, MOVE, TOUGH, MOVE, // 300
